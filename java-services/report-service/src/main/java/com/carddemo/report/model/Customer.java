...
/**
 * Customer entity mapped from COBOL copybook CVCUS01Y.cpy (CUSTOMER-RECORD, RECLN=500).
 */
@Entity
...
public class Customer {
    ...
    /**
     * Returns the full name formatted as in CBSTM03A: first middle last.
     */
    public String getFullName() {
        StringBuilder sb = new StringBuilder();
        if (custFirstName != null && !custFirstName.isBlank()) {
            sb.append(custFirstName.trim());
        }
        if (custMiddleName != null && !custMiddleName.isBlank()) {
            sb.append(" ").append(custMiddleName.trim());
        }
        if (custLastName != null && !custLastName.isBlank()) {
            sb.append(" ").append(custLastName.trim());
        }
        return sb.toString();
    }
}
