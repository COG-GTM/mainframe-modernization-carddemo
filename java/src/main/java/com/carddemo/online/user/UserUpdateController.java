...
/** COBOL program: COUSR02C (transaction CU02), BMS mapset COUSR02. */
@RestController
...
public class UserUpdateController {
    ...
    @GetMapping
    public UserDetailResponse updateScreen(HttpSession session) {
        CardDemoCommarea commarea = CommareaSession.find(session);
        if (commarea == null) {
            return returnToSignonScreen();
        }
        UserDetailResponse response =
                userUpdateService.initialScreen(commarea, UserAdminSession.require(session));
    ...
    public UserDetailResponse enter(@RequestBody UserFormRequest request, HttpSession session) {
        ...
        return userUpdateService.processEnterKey(request);
    }

    /** DFHPF5: save the updates. */
    @PostMapping("/pf5")
    public UserDetailResponse pf5(@RequestBody UserFormRequest request, HttpSession session) {
        if (CommareaSession.find(session) == null) {
            return returnToSignonScreen();
        }
        return userUpdateService.processPf5Key(request);
    }
    ...
    public UserDetailResponse pf4(HttpSession session) {
        ...
        return userUpdateService.processPf4Key();
    }

    /** DFHPF3: save the updates, then leave. */
    @PostMapping("/pf3")
    public UserDetailResponse pf3(@RequestBody UserFormRequest request, HttpSession session) {
        CardDemoCommarea commarea = CommareaSession.find(session);
        if (commarea == null) {
            return returnToSignonScreen();
        }
        UserDetailResponse response = userUpdateService.processPf3Key(request, commarea);
        CommareaSession.store(session, commarea);
        return response;
    }
    ...
    public UserDetailResponse pf12(HttpSession session) {
        ...
        UserDetailResponse response = userUpdateService.processPf12Key(commarea);
        CommareaSession.store(session, commarea);
        return response;
    }
    ...
    public UserDetailResponse otherKey(@RequestBody UserFormRequest request, HttpSession session) {
        ...
        return userUpdateService.processOtherKey(request);
    }
