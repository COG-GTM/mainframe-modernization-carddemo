...
/**
 * COBOL copybook: CSLKPCDY (lookup code repository) used by COBOL program COACTUPC
 * for phone area code, US state code and state/zip combination validation.
 */
public final class AccountLookupCodes {

    /** 88 VALID-GENERAL-PURP-CODE of WS-US-PHONE-AREA-CODE-TO-EDIT. */
    private static final Set<String> GENERAL_PURPOSE_AREA_CODES = unmodifiable(
    ...
    /** 88 VALID-US-STATE-CODE of US-STATE-CODE-TO-EDIT. */
    private static final Set<String> US_STATE_CODES = unmodifiable(
    ...
    /** 88 VALID-US-STATE-ZIP-CD2-COMBO of US-STATE-AND-FIRST-ZIP2. */
    private static final Set<String> STATE_ZIP2_COMBOS = unmodifiable(
    ...
    public static boolean isValidGeneralPurposeAreaCode(String areaCode) {
        return areaCode != null && GENERAL_PURPOSE_AREA_CODES.contains(areaCode.trim());
    }

    public static boolean isValidUsStateCode(String stateCode) {
        return stateCode != null && US_STATE_CODES.contains(stateCode.trim());
    }

    public static boolean isValidStateZipCombo(String stateCode, String zip) {
        if (stateCode == null || zip == null) {
            return false;
        }
        String state = stateCode.trim();
        String zipTrimmed = zip.trim();
        if (state.length() != 2 || zipTrimmed.length() < 2) {
            return false;
        }
        return STATE_ZIP2_COMBOS.contains(state + zipTrimmed.substring(0, 2));
    }
}
